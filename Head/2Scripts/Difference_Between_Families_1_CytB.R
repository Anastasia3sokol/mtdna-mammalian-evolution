...
#### get just 2 families 
GenFam = GenFam[GenFam$ECO.Family == c('Cervidae','Dasypodidae'),]
    ...
    for (sp2 in (sp1+1):(NumberOfSp-1)){
      ...
      for(sp3 in (sp2+1):NumberOfSp){
        ...
        for (i in 1:length(VecOfNuc2)/3){
          ...
          Codons3 = paste(Codons3, collapse = '')
          if (Codons2 != Codons3){
            DifBtwSp2AndSp3 = paste(DifBtwSp2AndSp3, StartNuc,':', Codons2, '>', Codons3, ';', sep = "")
          }
          StartNuc = StartNuc + 3
        }
...
setwd(wd)
write.table(DifferenceBetweenSpecies, file = "DifferenceBetweenSpeciesFamilies1CytB.txt")
