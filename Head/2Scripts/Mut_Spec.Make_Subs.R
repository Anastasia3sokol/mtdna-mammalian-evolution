...
makemut <- function(x)
{
  ...
  for (c in 1:(length(VecOfNucOut)/3))
  {
      ...
      if (A1 == A2 & A1 == A3 & A2 == A3)
      {
            ...
            & CodonsOut %in% VecOfSynFourFoldDegenerateSites) 
        {
          ...
          {#n = 1
            ...
            if(NucIn0[n] != NucIn1[n]) {DifIn0In1 = DifIn0In1+1}
            if(NucIn0[n] != NucOut[n]) {DifIn0Out = DifIn0Out+1}
            if(NucIn1[n] != NucOut[n]) {DifIn1Out = DifIn1Out+1}
      
          }
          if (DifIn0In1 == 1 & DifIn0Out == 0)
          {
            Subs = paste(NucIn0[3], '>', NucIn1[3], sep='')
          ...
          {
            Subs = paste(NucIn1[3], '>', NucIn0[3], sep='')
...
#### save table
write.table(trios_seq,file = '../../Body/2Derived/MutSpec.Make_Subs.txt')
